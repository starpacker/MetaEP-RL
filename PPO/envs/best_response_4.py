...
_best_records = []
_simulation_count = 0  # 总仿真次数（仅用于记录，不用于终止）
...
def _run_data(pra_short):
    ...
    _simulation_count += 1  # 仅用于全局计数，不影响终止逻辑
    ...
    return _reward_7(stru, pra_short)
...
def _reward_7(data, pra):
    ...
    if reward > _best_rewards:
        ...
        with open("C:\\data\\best_result_br_2.txt", 'w') as f:
            f.write(f"m:{m}\n")
            f.write(f"CD:{c}\n")
...
def _single_parameter_search(pra_base, search_index, search_range, max_sim_per_param, description="Single Parameter Search"):
    """
    对单个参数进行搜索，最多尝试 max_sim_per_param 次仿真（包括无效结构）。
    """
    global _best_rewards, _best_records, _simulation_count

    print(f"[{description}] Searching parameter index {search_index}")
    print(f"Search range: {search_range}")
    print(f"Max simulations for this parameter: {max_sim_per_param}")

    best_local_pra = pra_base.copy()
    best_local_reward = -np.inf
    local_sim_count = 0  # 本参数的仿真计数器

    for val in search_range:
        if local_sim_count >= max_sim_per_param:
        ...
        if not boundary_test(current_pra):
            ...
            local_sim_count += 1  # 只有调用 _run_data 才算一次仿真（你也可以选择对无效结构也计数）

        # 注意：如果你希望“无效结构”也算一次尝试（即计入 max_sim_per_param），则把 local_sim_count +=1 移到 if 之前
        # 目前设计：仅有效结构（调用仿真）才计数。若需包含无效结构，请调整如下：

        # === 如果你希望所有尝试（包括无效）都计入上限，请用以下逻辑替代上面的 if/else ===
        # local_sim_count += 1
        # if not boundary_test(current_pra):
        #     reward = _out_of_bound_reward
        #     print(f"[{description}] Invalid structure: {current_pra}, reward: {reward}")
        ...
        print(f"[{description}] Parameters: {current_pra}, Reward: {reward}, "
              f"local_count: {local_sim_count}, total_count: {_simulation_count}")
...
def run_best_response_search(initial_pra, search_indices, max_simulations_per_param=None):
    ...
    Args:
        ...
        max_simulations_per_param (int or None): 每个参数的最大仿真次数。None 表示无限制（遍历整个 search_range）
    ...
    open("C:/data/all_rewards_7.txt", "w").close()  # 清空历史记录文件

    current_pra = list(initial_pra)

    print("=== 单阶段 Best Response 搜索（步长=4）===")
    ...
    for idx in search_indices:
        ...
        # 如果未设置上限，则使用整个 range
        actual_max = max_simulations_per_param if max_simulations_per_param is not None else len(search_range)

        best_pra, best_reward = _single_parameter_search(
            current_pra, idx, search_range,
            max_sim_per_param=actual_max,
            description=f"Step=4 Search - Parameter {idx}"
        )
