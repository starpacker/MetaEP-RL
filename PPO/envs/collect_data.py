...
def runlume(
             ...
             model_file="jones_model_origin.fsp",
...
# wavelength of 642nm
def reward(data,alpha=0.2,base=0.8,beta=0.2):
    wave_l = 88
    # wave_l = 84
    global best_rewards
    ...
    # print("wavelength:",wave_length[wave_l])
    ...
    # real_ep_wave = np.argmin(dis_real)
    # real_ep = np.min(dis_real)
    # real_imag_wave = np.argmin(dis_imag)
    # imag_ep = np.min(dis_imag)
    CD = abs(r_lr - r_rl) / (r_lr + r_rl + 2 * r_rr_ll)
    # mean_ep = (imag_ep + real_ep)/2
    # min_imag_ep = [a for a,b in enumerate(dis_imag) if b<=0.01]
    # min_real_ep = 10
    r = dis_real[wave_l]
    i = dis_imag[wave_l]
    m = (r + i)/2
    ...
    if reward > best_rewards:
        best_rewards  = reward
        with open(f"C:\\data\\best_result.txt", 'w') as f:
            f.write(f"m:{m}\n")
            f.write(f"CD:{c}\n")
        ...
        # 保存 best reward 到文件
        np.savetxt(f"C:\\data\\records_{judge_flag}.txt", np.array(best_records), fmt='%d %.6f')

    # write CD history
    file = open(f"C://data//CD_history.txt", 'a')
...
def run_onetime(pra):
    ...
    if os.path.exists(filepath):
        ...
        np.savetxt('C:\\data\\pra.txt',pra)
        runlume()
        if not os.path.exists("C:\\data\\farfile_reflection.txt"):
            
            simulation_count += 1 
            return out_of_bound_reward
        data = np.loadtxt("C:\\data\\farfile_reflection.txt")
        np.savetxt(filepath, data)
        os.remove("C:\\data\\farfile_reflection.txt")  # 删除文件
...
if judge_flag:
    ...
    # from judge.unet import UNetRegressor
    # judger_model = UNetRegressor().to(device)
    # judger_model.load_state_dict(torch.load("C:/light_mappo-main-2/judge/best_model_unet.pth"))
    # judger_model.eval()
...
def run_onetime_judge(pra):
    ...
    if os.path.exists(filepath):
        ...
        # 否则继续执行 FDTD 模拟
        np.savetxt('C:\\data\\pra.txt', pra_short)
        runlume() 
        data = np.loadtxt("C:\\data\\farfile_reflection.txt")
        os.remove("C:\\data\\farfile_reflection.txt")  # 删除文件
        flag_2 = True
    ...
    return reward(stru) * 10
    # return reward(stru) / 5

def run_onetime_total_new(pra):  # 这个函数选择不依靠任何老数据，纯依赖judger和FDTD进行PPO算法
    global flag_2
    pra_short = pra[:10]  
    ...
    # 生成逗号分隔的文件名
    # filename = "C:\\data\\pra\\" + ",".join(map(str, pra_short)) + ".txt"

    # os.rename("C:\\data\\farfile_reflection.txt", filename)
    flag_2 = True
...
# 更改env_core.py里面环境重启的内容

def get_reward():
    reward_all = []
    pras = np.loadtxt("C:\\data\\pra_all.txt")
    pra_int = pras.astype(int)
    if pra_int[0] == 0:
        reward_all.append(out_of_bound_reward)   #  越界给超低奖励
    else:
        if not judge_flag:
            reward_all.append(run_onetime(pra_int))
        else:
            # reward_all.append(run_onetime_total_new(pra_int))
            reward_all.append(run_onetime_trans(pra_int))

    file = open(f"C://data//reward_history.txt", 'a')

    for w in reward_all:
        file.writelines(str(pra_int)+'\n')
        file.writelines(str(w)+'\n')
    file.close()
    np.savetxt(f"C:\\data\\reward.txt",np.array(reward_all))
