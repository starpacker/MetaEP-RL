...
def reward_3(data):
    ...
    if reward > best_rewards:
        ...
        # 保存 best reward 到文件
        np.savetxt(f"C:\\data\\records_{judge_flag}_3.txt", np.array(best_records), fmt='%d %.6f')
...
if judge_flag:
    ...
    # judger_model.load_state_dict(torch.load("C:/light_mappo-main-3/judge/best_model_0719.pth"))
    # judger_model.eval()

    # from judge.FCN import FCNRegressor
    # judger_model = FCNRegressor().to(device)
    # judger_model.load_state_dict(torch.load("C:/light_mappo-main-3/judge/best_model_fcn_0729.pth"))
    # judger_model.eval()

    from judge.transformer import EdgeLengthTransformerRegressor
    judger_model = EdgeLengthTransformerRegressor().to(device)
    judger_model.load_state_dict(torch.load("C:/light_mappo-main-3/judge/best_model_trans_0821.pth"))
    judger_model.eval()
    # 参数边界定义
    param_bounds = {
        0: (30, 160),
...
def run_onetime_total_new_3(pra):  # 这个函数选择不依靠任何老数据，纯依赖judger和FDTD进行PPO算法
    global flag_3
    global simulation_count
    pra_short = pra[:10]  

    # 生成 pattern
    pattern = get_pattern_3(pra_short)
    # 转换为模型输入格式 (B, C, H, W)
    ...
    # 否则继续执行 FDTD 模拟
    np.savetxt('C:\\data\\pra_3.txt', pra_short)
    runlume_3()
    if not os.path.exists("C:\\data\\farfile_reflection_3.txt"): 
        simulation_count += 1 
        return out_of_bound_reward
...
def run_onetime_trans(pra):  # 注意：此函数名未变，但内部调用的是 _3 版本
    global flag_3
    global simulation_count
    pra_short = pra[:10]  
...
def get_reward():
    ...
    if pra_int[0] == 0:
        ...
        if not judge_flag:
            ...
            # reward_all.append(run_onetime_total_new_3(pra_int))
            reward_all.append(run_onetime_trans(pra_int))

    file = open("C://data//reward_history_3.txt", 'a')
