...
class AnalysisSamples:
    ...
    def __init__(self, data_path,target=100):
        # 参数初始化
...
# 由多个小矩形组成的结构类
# 初始化结构大小，eg(40*40)，代表400*400nm
# rect:为输入向量：[xmin, xmax, ymin, ymax]
# 注意numpy的转置问题
class PatternRects:
    # 参数初始化
    ...
    # 对结构进行描述
    def get_details(self,op,op_show=False):
...
class FDTDModel:
    ...
    def copy_and_modify_lsf(self):
        ...
        lsf_scipt = os.path.join(self.file_root, "script_copy.lsf")
    ...
    def run_lumerical(self, lsf_scipt):
        ...
        fdtd_solutions = 'C:\Program Files\Lumerical\\v202\\bin\\fdtd-solutions.exe'
        fsp_file = self.model
        lsf_file = lsf_scipt
