...
def run_best_response_search(initial_pra, search_indices):
    ...
    for idx in search_indices:
        ...
        start = max(min_val, best_val - search_branch)
        end = min(max_val, best_val + search_branch)
...
# to run this independently, we need to randomly initialze the pra:
# [None,None,52,152,48,48,None,None,80,76]
# random choose 0,1,6,7 pra and if it passes the boundary test, 
# then use it as initial pra
# repeatedly run run_best_response_search, until the total simulation count reaches 2000
