...
def parse_args(args, parser):
    parser.add_argument("--scenario_name", type=str, default="mappo-750", help="Which scenario to run on")
...
def main(args):
    ...
    def save_config(save_dir):
        """
        保存当前训练的配置参数到指定目录下的 JSON 文件中。
        文件名包含时间戳，避免重复。
        """
        os.makedirs(save_dir, exist_ok=True)
        args_dict = vars(all_args)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        save_path = os.path.join(save_dir, f"config_{timestamp}.json")
